...
class SearchBloc extends Bloc<SearchEvent, SearchState> {
  ...
  static SearchBloc get get => BlocProvider.of(navigatorKey.currentState!.context);

  ///Action to search on items.
  FutureOr<void> _searchOnItems(SearchOnItemsEvent event, Emitter<SearchState> emit) async{
    emit(state.copyWith(searchValue: event.searchValue,));
    if (event.isSeeMore) {
      emit(state.copyWith(currentSearchPage: state.currentSearchPage + 1,searchStateStatus: SearchStateStatus.LOADINGMORE));
    }
    else{
