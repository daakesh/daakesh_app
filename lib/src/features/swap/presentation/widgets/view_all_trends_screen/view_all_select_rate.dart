...
class ViewAllSelectRate extends StatelessWidget {
  const ViewAllSelectRate({super.key});
...
class ViewAllStarType extends StatelessWidget {
  final int index;
  final String title;
  final bool isAllType;

  const ViewAllStarType({
    super.key,
    required this.index,
    required this.title,
  ...
  @override
  Widget build(BuildContext context) {
    return BlocBuilder<TrendDealsBloc, TrendDealsState>(
        builder: (context, state) {
      return isAllType
          ? Expanded(
              child: GestureDetector(
                onTap: () => TrendDealsBloc.get
                    .add(SetViewAllFilterDataEvent(rate: index)),
                child: Container(
                  width: 50.0,
