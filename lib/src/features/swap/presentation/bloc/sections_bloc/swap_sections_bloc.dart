...
class SwapSectionsBloc extends Bloc<SwapSectionsEvent, SwapSectionsState> {
  ...
  static SwapSectionsBloc get get => BlocProvider.of(navigatorKey.currentState!.context);

  ///Event to get sections categories at [SectionScreen].
  FutureOr<void> _getCategoryBySectionID(SwapGetCategoryBySectionIDEvent event, Emitter<SwapSectionsState> emit)async {
    if (event.isSeeMore) {
      emit(state.copyWith(
    ...
    final result = await getIt.get<SwapUseCases>().getCategoryBySectionID(state.secID,state.currentPage);
    result.fold((l) {
      emit(state.copyWith(swapSectionsStateStatus: SwapSectionsStateStatus.ERROR));
      ...
      List<SwapCategoryItem> newResultList = categoryData.data!.categoryItemList!.toList();
      List<SwapCategoryItem> categoriesListData = state.swapCategoriesListData.toList();
  ...
  FutureOr<void> _resetVarEvent(SwapResetVarEvent event, Emitter<SwapSectionsState> emit) {
    emit(state.copyWith(swapSectionsStateStatus: SwapSectionsStateStatus.SUCCESS,
      sectionIndex: -1,
      currentPage: 1,
      isMoreData:true,
      swapCategoriesListData: [],
