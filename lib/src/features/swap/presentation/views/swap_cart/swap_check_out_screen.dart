import 'package:flutter/material.dart';
import 'package:persistent_bottom_nav_bar/persistent_tab_view.dart';
import '../../../../../src.export.dart';

class SwapCheckOutScreen extends StatelessWidget {
  const SwapCheckOutScreen({super.key});

  @override
  Widget build(BuildContext context) {
    return Scaffold(
      body: LayoutBuilderWidget(
        child: Column(
          children: [
            const HeaderWidget(),
            const SizedBox(
              height: 25.0,
            ),
            const SwapYourCartItemWidget(),
            const SizedBox(
              height: 14.0,
            ),
            const SwapShippingLocationWidget(),
            const SizedBox(
              height: 25.0,
            ),
            Padding(
              padding: const EdgeInsets.symmetric(horizontal: 27.0),
              child: Row(
                children: [
                  Text(
                    'Total : ',
                    style: context.easyTheme.textTheme.labelLarge!
                        .copyWith(fontSize: 28.0, color: ColorName.black),
                    overflow: TextOverflow.ellipsis,
                  ),
                  const Spacer(
                    flex: 1,
                  ),
                  Text(
                    '\$88.6',
                    style: context.easyTheme.textTheme.labelLarge!
                        .copyWith(fontSize: 28.0, color: ColorName.black),
                  ),
                ],
              ),
            ),
            const SizedBox(
              height: 12.0,
            ),
            const Spacer(
              flex: 1,
            ),
            Padding(
              padding: const EdgeInsets.symmetric(horizontal: 21.5),
              child: DefaultButtonWidget(
                  text: 'PAY', onPressed: () => openCartPayScreen(context)),
            ),
            const SizedBox(
              height: 50.0,
            ),
          ],
        ),
      ),
    );
  }

  void openCartPayScreen(context) async {
    PersistentNavBarNavigator.pushNewScreen(
      context,
      screen: const SwapCartPayScreen(),
      withNavBar: true, // OPTIONAL VALUE. True by default.
    );
  }
}
