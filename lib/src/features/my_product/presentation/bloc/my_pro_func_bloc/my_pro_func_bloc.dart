...
class MyProFuncBloc extends Bloc<MyProFuncEvent, MyProFuncState> {
  ...
  Future<FutureOr<void>> _swapTabBarProductType(
      SwapTabBarProductTypeEvent event, Emitter<MyProFuncState> emit) async {
  ...
  FutureOr<void> _chooseProductDisplayMethod(
      ChooseProductDisplayMethodEvent event, Emitter<MyProFuncState> emit) {
    emit(state.copyWith(productDisplayMethod: event.productDisplayMethod));
  }

  ///Event to select Swap country at [ForSwapScreen]
  FutureOr<void> _changeCountrySwapFlag(
      ChangeCountrySwapFlagEvent event, Emitter<MyProFuncState> emit) {
    emit(state.copyWith(swapFlagEmoji: event.flagEmoji));
  }

  ///Select ship to countries at [ShipToScreen]
  FutureOr<void> _selectShipToCountry(
      SelectShipToCountryEvent event, Emitter<MyProFuncState> emit) {
  ...
  FutureOr<void> _deleteInsertCountries(
      DeleteInsertCountriesEvent event, Emitter<MyProFuncState> emit) {
  ...
  FutureOr<void> _checkAlreadyAddedCountry(
      CheckAlreadyAddedCountryEvent event, Emitter<MyProFuncState> emit) {
  ...
  FutureOr<void> _resetValues(
      ResetValuesEvent event, Emitter<MyProFuncState> emit) {
    emit(
      state.copyWith(
  ...
  FutureOr<void> _addProductImage(
      AddProductImageEvent event, Emitter<MyProFuncState> emit) {
  ...
  FutureOr<void> _editProduct(
      EditProductEvent event, Emitter<MyProFuncState> emit) {
    MyProFuncBloc.get.add(ChooseProductDisplayMethodEvent(
        productDisplayMethod: event.productDisplayMethod));
    GetItUtils.editProduct.setData(event.myProductItem);
  ...
  FutureOr<void> _searchOnProduct(
    ...
    result.fold((l) {
      ...
      if (newResultList.isEmpty) {
        emit(state.copyWith(
          myProFuncStateStatus: MyProFuncStateStatus.NULL,
  ...
  FutureOr<void> _emptyProductSearch(
      EmptyProductSearchEvent event, Emitter<MyProFuncState> emit) {
    emit(state.copyWith(
        myProFuncStateStatus: MyProFuncStateStatus.SUCCESS,
        searchResultList: [],
