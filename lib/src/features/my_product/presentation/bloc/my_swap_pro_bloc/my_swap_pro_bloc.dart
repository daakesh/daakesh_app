...
class MySwapProBloc extends Bloc<MySwapProEvent, MySwapProState> {
  ...
  static MySwapProBloc get get => BlocProvider.of(navigatorKey.currentState!.context);

  FutureOr<void> _getMySwapPro(GetMySwapProEvent event, Emitter<MySwapProState> emit) async{
    if (event.isSeeMore) {
      emit(state.copyWith(
    ...
    final result = await getIt.get<MyProductUseCases>().getMyProduct(state.currentPage,UserType.SWAP.name);
    ...
    result.fold((l) {
      ...
      List<MyProductItem> myProductListData= state.mySwapProductListData.toList();
