...
class ContactInfoBloc extends Bloc<ContactInfoEvent, ContactInfoState> {
  ...
  static ContactInfoBloc get get => BlocProvider.of(navigatorKey.currentState!.context);

  FutureOr<void> _activateUpdateContactInfo(ActivateUpdateContactInfoEvent event, Emitter<ContactInfoState> emit) {
    emit(state.copyWith(isUpdatePersonalActive: event.isUpdatePersonalActive));
  }
  FutureOr<void> _resetContactInfo(ResetContactInfoEvent event, Emitter<ContactInfoState> emit) {
    emit(state.copyWith());
  }
  FutureOr<void> _getContactInfo(GetContactInfoEvent event, Emitter<ContactInfoState> emit) async{
    emit(state.copyWith(contactInfoStateStatus:  ContactInfoStateStatus.LOADING));
    final result = await getIt.get<ProfileUseCases>().getContactInfo();
    result.fold((l) {
      emit(state.copyWith(contactInfoStateStatus: ContactInfoStateStatus.ERROR));
      ...
      emit(state.copyWith(contactInfoStateStatus: ContactInfoStateStatus.SUCCESS));
    });
  }
  FutureOr<void> _addContactInfo(AddContactInfoEvent event, Emitter<ContactInfoState> emit)async {
    emit(state.copyWith(contactInfoStateStatus:  ContactInfoStateStatus.LOADING));
  ...
  FutureOr<void> _editContactInfo(EditContactInfoEvent event, Emitter<ContactInfoState> emit) {
    emit(state.copyWith(
